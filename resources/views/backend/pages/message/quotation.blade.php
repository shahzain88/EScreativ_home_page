                            ...
                            <table  id="esDatatable" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($quotations as $key => $quotation)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>{{ $quotation->name }} <br>{{ $quotation->mobile }} <br>
                                                {{ $quotation->email }} </td>
                                            <td>
                                                @if ($quotation->quotation)
                                                    <span class="badge badge-primary">Quotation</span>
                                                @endif
                                                @if ($quotation->visit)
                                                    <span class="badge badge-primary">Visit</span>
                                                @endif
                                                @if ($quotation->diagnosis)
                                                    <span class="badge badge-primary">Diagnosis</span>
                                                @endif
                                            ...
                                            <th>
                                                ...
                                                <form style="display: inline-block" class="deleteQuotationForm"  method="POST">
                                                    @method('DELETE')
                                                    @csrf
                                                    <input type="hidden" class="deleteId" name="id" value="{{ $quotation->id }}">
                                                    <input type="hidden" class="deleteUrl" name="url" value="{{route('quotation.destroy', $quotation->id)}}">
