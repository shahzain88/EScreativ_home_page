                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-right">
                            <a class="btn btn-info" href="{{ route('product.create') }}">Create New</a>
                        </div>
                    </div>
                </div>
                <!-- /.card-header -->
                    ...
                    <tbody>
                      @if ($products)
                        @foreach ($products as $key=>$product)
                          ...
                          <td>{{substr($product->product_details,0,10)}}...</td>
                          <td>{{$product->service_name}}</td>

                          <td>{{substr($product->service_description,0,10)}}...</td>

                        <!--<td>-->
                        <!--      <img src="{{asset('/')}}{{$product->product_image}}" height="100" width="100" alt="Preview">-->
                        <!--</td>-->
                            <td>
                              @if ($product->status == 0)
                          ...
                          <td>
                                <a class="btn btn-info" href="{{route('product.edit', $product->id)}}"><i class="fa fa-edit"></i> Edit</a>

                                <form style="display: inline-block" class="deleteProductForm"  method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <input type="hidden" class="deleteId" name="id" value="{{ $product->id }}">
                                    <input type="hidden" class="deleteUrl" name="url" value="{{route('product.destroy', $product->id)}}">
